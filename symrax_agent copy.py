...
class Assistant(Agent):
    """Basic voice assistant with Symrax booking capabilities."""

    def __init__(self):
        super().__init__(
            instructions="""You are a helpful and friendly Symrax voice assistant.
            You can help users search for Symraxs in different cities and book their stays.
            Keep your responses concise and natural, as if having a conversation."""
        )

        # Mock Symrax database
        self.Symraxs = {
            "san francisco": [
                {
                    "id": "sf001",
    ...
    @function_tool
    async def search_Symraxs(self, context: RunContext, city: str) -> str:
        """Search for available Symraxs in a city.

        Args:
            city: The city name to search for Symraxs (e.g., 'San Francisco', 'New York', 'Los Angeles')
        """
        city_lower = city.lower()

        if city_lower not in self.Symraxs:
            return f"Sorry, I don't have any Symrax listings for {city} at the moment. Available cities are: San Francisco, New York, and Los Angeles."

        listings = self.Symraxs[city_lower]
        result = f"Found {len(listings)} Symraxs in {city}:\n\n"
    ...
    @function_tool
    async def book_Symrax(self, context: RunContext, Symrax_id: str, guest_name: str, check_in_date: str, check_out_date: str) -> str:
        """Book an Symrax.

        Args:
            Symrax_id: The ID of the Symrax to book (e.g., 'sf001')
        ...
        # Find the Symrax
        Symrax = None
        for city_listings in self.Symraxs.values():
            for listing in city_listings:
                if listing['id'] == Symrax_id:
                    Symrax = listing
                    break
            if Symrax:
                break
        ...
        if not Symrax:
            return f"Sorry, I couldn't find an Symrax with ID {Symrax_id}. Please search for available listings first."

        # Create booking
        booking = {
            "confirmation_number": f"BK{len(self.bookings) + 1001}",
            "Symrax_name": Symrax['name'],
            "address": Symrax['address'],
            "guest_name": guest_name,
            "check_in": check_in_date,
            "check_out": check_out_date,
            "total_price": Symrax['price'],
        }
        ...
        result += f"Property: {booking['Symrax_name']}\n"
